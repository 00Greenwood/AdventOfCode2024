...
std::string Day9::solve(Part part) const {
  ...
  for (int i = 0; i < m_input.size(); i++) {
    ...
    if (i % 2 == 0) {
      for (size_t j = 0; j < m_input.at(i); ++j) {
  ...
  if (part == PART_1) {
    ...
    for (size_t i = layout.size() - 1; i >= 0; --i) {
      start = std::find(start, layout.end(), -1);
      if (start != layout.end() && i > static_cast<size_t>(start - layout.begin())) {
  ...
  if (part == PART_2) {
    ...
    for (size_t i = partitions.size() - 1; i >= 0; --i) {
      ...
      if (it != partitions.end() && i > static_cast<size_t>(it - partitions.begin())) {
        std::vector<int>& space = partitions.at(it - partitions.begin());
        if (file.size() == space.size()) {
          std::swap(file, space);
        } else {
          std::vector<int> difference;
          for (size_t j = file.size(); j < space.size(); ++j) {
